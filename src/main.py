...
import argparse
from SRAFL_model import DFSNet, SetEncoder, MLP, DuelingNet
import csv
import os
import random
import pprint
...
import pickle as pk
from agent_environment import multirange
...
def run_n_steps(step_runner, env, n_steps, eps=None, mode='double'):
    ''' Obatin history data for training
    '''
...
def test_and_record(step_runner, args, env, valenv=None, testenv=None):
    ...
    'n_acquired_max', 'n_acquired_med',  'return','acquired']
    n_features, n_classes = env.n_features, env.n_classes
    if n_classes == 2:
    ...
    for prefix, environ in [('val', valenv), ('tr', env), ('ts', testenv)]:
        ...
        result[prefix + '_acquired'] = acquired[5:15,:]
...
def main(args):
  ...
  # Firstly loading data, defining classifier and declaring env
    ...
    learn(step_runner, args, env, valenv, nsteps=nsteps,
            total_steps=int(1e5), scheduler=args.scheduler)
    learn_elapsed = time() - learn_start
    print("Ending1- learning time")
    print(learn_elapsed)
    print("Ending2- val results")
    dfsnet.eval()
    print("Ending3- test results")
    test_and_record(step_runner, args, env, valenv, testenv)
    print("train-val-test ending!")
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--disable_cuda', action='store_true', default='True',help='Disable CUDA')
    ...
    parser.add_argument(
        '--data_type', help='data', type=str, default='CSV'
    )
    ...
    parser.add_argument(
        '--batch_size', help='batch size', type=int, default=4#default=128
    )
